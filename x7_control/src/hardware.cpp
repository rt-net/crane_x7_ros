...
/**
 * @fn		void Move_Goal_Current()
 * @brief	設定してあるGoal Currentへ移動
 * @param	goal_current[8](static double goal_current[8]) サーボの個数分のデータ(deg)
 */
void CR7::Move_Goal_Current( double *goal_current){
	//Move target goal position
	for(int i=0;i<JOINT_NUM;i++){
		printf("[ ID[%d] : %lf ]", ID[i], goal_current[i]);																				//指定したサーボとデータの確認
		if((JOINT_MIN[i] > deg2value(goal_pose[i])) || (JOINT_MAX[i] < deg2value(goal_pose[i]))){										//動作角度外の角度が入力された場合
			printf("over range!¥n");																										//入力データがdegなのでvalueに変換(中心が0になる)
			break;
		}

		param_goal_current[0]  = DXL_LOBYTE(goal_current[i]);
		param_goal_current[1]  = DXL_HIBYTE(goal_current[i]);

		dxl_addparam_result = groupBulkWrite->addParam(ID[i], ADDR_PRO_GOAL_CURRENT, LEN_PRO_GOAL_CURRENT, param_goal_current);		//書き込み用のパケットに追加
		if(dxl_addparam_result != true) printf("goal pose error!¥n");
...
bool CR7::Teaching_Play_Frame(){
	...
	//while(1){																															//データ取得の開始
		...
		for(int i=0;i<JOINT_NUM;i++){
			...
			joint_pose[i] = (value2deg(dxl_present_position));
